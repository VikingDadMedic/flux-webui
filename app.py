...
def infer(
    ...
    if selected != checkpoint:
        if checkpoint == "sayakpaul/FLUX.1-merged":
            ...
            if device == "mps":
                transformer = QuantizedFluxTransformer2DModel.from_pretrained(
                    "cocktailpeanut/flux1-merged-qint8"
                )
            else:
        ...
        pipe = FluxPipeline.from_pretrained(
            bfl_repo, transformer=None, torch_dtype=dtype
        )
        print("initialized!")
...
with gr.Blocks(css=css) as demo:
    with gr.Column(elem_id="col-container"):
        gr.HTML(
            "<nav><img id='logo' src='https://d1w5f3vqncwk8i.cloudfront.net/52a53afa-6620-4280-91a9-9f1171845a5f.png'/></nav>"
        )
        with gr.Row():
            prompt = gr.Text(
                label="Prompt",
        ...
        result = gr.Gallery(
            label="Result", show_label=False, object_fit="contain", format="png"
        )
        ...
        checkpoint.change(
            fn=update_slider, inputs=[checkpoint], outputs=[num_inference_steps]
        )
    gr.on(
